...
func TestLSLinkHandler_Add(t *testing.T) {
	...
	// Both stub nodes should exist.
	if g.GetVertex("0000.0000.0001") == nil {
		t.Error("local node 0000.0000.0001 not found")
	}
	if g.GetVertex("0000.0000.0002") == nil {
		t.Error("remote node 0000.0000.0002 not found")
	}

	// Interface vertex.
	ifID := "iface:0000.0000.0001/10.0.0.1"
	if g.GetVertex(ifID) == nil {
		t.Errorf("interface vertex %q not found", ifID)
	}
	...
	e := g.GetEdge(edgeID)
	if e == nil {
		t.Fatalf("link edge %q not found", edgeID)
	}
